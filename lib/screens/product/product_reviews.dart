...
import 'package:neurom_bilisim_store/repositories/review_repositories.dart';
...
class ProductReviews extends StatefulWidget {
  ...
  ProductReviews({super.key, this.id, this.productId, this.productName});
...
class _ProductReviewsState extends State<ProductReviews> {
  ...
  fetchData() async {
    var reviewResponse = await ReviewRepository().getReviewResponse(
      widget.productId ?? widget.id,
      page: _page,
    );
    
    // İlk sayfa ise listeyi temizle, değilse ekle
    if (_page == 1) {
      _reviewList.clear();
    }
    
    // Duplikasyon kontrolü - aynı yorum ID'si varsa ekleme
    if (reviewResponse.reviews != null) {
      for (var review in reviewResponse.reviews!) {
        // Aynı ID'ye sahip yorum varsa ekleme
        bool alreadyExists = _reviewList.any((existingReview) => 
  ...
  onTapReviewSubmit(context) async {
    ...
    print("Yorum gönderme başlatılıyor...");
    print("Product ID: ${widget.productId ?? widget.id}");
    print("Rating: ${_my_rating.toInt()}");
    print("Comment: $myReviewText");
    ...
    print("Yorum gönderme response result: ${reviewSubmitResponse.result}");
    print("Yorum gönderme response message: ${reviewSubmitResponse.message}");
